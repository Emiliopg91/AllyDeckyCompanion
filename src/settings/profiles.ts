...
export class Profiles {
    ...
    public static getProfileForId(id: string): Profile {
        if (!Profiles.existsProfileForId(id)) {
            ...
            const mode = id.endsWith(Constants.SUFIX_AC) ? Constants.TDP_AC_DEFAULT_MODE : Constants.TDP_DEFAULT_MODE
            ...
            const profile: Profile = {
                ...
                gpu: {
                    frequency: {
                        min: tmpProf.gpu.frequency.min,
                        max: tmpProf.gpu.frequency.max
                    }
                }
            }
            ...
            return {
                ...
                gpu: {
                    frequency: {
                        min: Math.max(WhiteBoardUtils.getGpuMinFreq(), Number(Settings.getEntry(Constants.PREFIX_PROFILES + id + Constants.SUFIX_GPU_FREQ_MIN, String(WhiteBoardUtils.getGpuMinFreq())))),
                        max: Math.min(WhiteBoardUtils.getGpuMaxFreq(), Number(Settings.getEntry(Constants.PREFIX_PROFILES + id + Constants.SUFIX_GPU_FREQ_MAX, String(WhiteBoardUtils.getGpuMaxFreq()))))
                    }
                }
            }
        }
    ...
    public static getProfileForMode(mode: Mode): Profile {
        ...
        switch (mode) {
            case Mode.SILENT:
                profile.cpu.tdp.spl = WhiteBoardUtils.getIsAllyX() ? Constants.AllyXSilentSPL : Constants.AllySilentSPL
                profile.cpu.tdp.sppl = Constants.AllySilentSPPL
                profile.cpu.tdp.fppl = Constants.AllySilentFPPL
                break;
            case Mode.PERFORMANCE:
            case Mode.CUSTOM:
                profile.cpu.tdp.spl = WhiteBoardUtils.getIsAllyX() ? Constants.AllyXPerformanceSPL : Constants.AllyPerformanceSPL
                profile.cpu.tdp.sppl = Constants.AllyPerformanceSPPL
                profile.cpu.tdp.fppl = Constants.AllyPerformanceFPPL
                break;
            case Mode.TURBO:
                profile.cpu.tdp.spl = WhiteBoardUtils.getIsAllyX() ? Constants.AllyXTurboSPL : Constants.AllyTurboSPL
    ...
    public static saveProfileForId(id: string, profile: Profile) {
        ...
        Settings.setEntry(Constants.PREFIX_PROFILES + id + Constants.SUFIX_GPU_FREQ_MIN, String(profile.gpu.frequency.min), true)
        Settings.setEntry(Constants.PREFIX_PROFILES + id + Constants.SUFIX_GPU_FREQ_MAX, String(profile.gpu.frequency.max), true)
    }
