...
const checkProfilePerGame = (): Promise<void> => {
  return new Promise<void>((resolve) => {
    if (!Settings.getEntry(Constants.PROFILE_PER_GAME)) {
      ...
      WhiteBoardUtils.setProfilePerGame(
        Settings.getEntry(Constants.PROFILE_PER_GAME) == "true",
      );
      resolve();
    }
  });
...
const checkPluginLatestVersion = async (): Promise<void> => {
  try {
    ...
    if (
      vers != WhiteBoardUtils.getPluginLatestVersion() &&
      Constants.PLUGIN_VERSION != vers
    ) {
  ...
  return new Promise<void>((resolve) => {
    if (WhiteBoardUtils.getIsAlly()) {
      BackendUtils.getAvailableGovernors().then((governors) => {
        ...
        Logger.info(
          "Available scaling governors: " + JSON.stringify(governors),
        );
        resolve();
      });
    } else {
...
const migrateSchema = (): void => {
  Logger.info(
    "Migrating settings file schema to v" + Constants.CFG_SCHEMA_VERS,
  );
    ...
    Object.keys(profiles[appId]).forEach((pwr) => {
      if (typeof profiles[appId][pwr] != "string") {
        ...
        if (!profile.cpu?.governor) {
          Settings.setEntry(
            Constants.PREFIX_PROFILES +
              appId +
              "." +
              pwr +
              Constants.SUFIX_CPU_GOVERNOR,
            "powersave",
            true,
          );
        }
  ...
  (async (): Promise<void> => {
    ...
    checkSdtdp().then(() => {
      checkProfilePerGame().then(() => {
        Logger.info(
          "Profile per-game " +
            (WhiteBoardUtils.getProfilePerGame() ? "en" : "dis") +
            "abled",
        );
            ...
            getBiosVersion().then(() => {
              getAvailableGovernors().then(() => {
                getGpuRanges().then(() => {
                  ...
                  Logger.info(
                    "Mode ONLY_GUI " +
                      (WhiteBoardUtils.getOnlyGui() ? "en" : "dis") +
                      "abled",
                  );
                  ...
                  sleep(5000).then(() => {
                    ...
                    checkPluginLatestVersion();
                    sleep(1000).then(() => {
                      biosUpdateCheckTimer = setInterval(
                  ...
                  runningGameIdUnregister = EventBus.subscribe(
                    ...
                    (e: EventData) => {
                      if (
                        (e as WhiteBoardEventData).getId() == "runningGameId"
                      ) {
                        Profiles.applyGameProfile(
                          (e as WhiteBoardEventData).getValue(),
                        );
                      }
                    },
                  ).unsubscribe;
                  ...
                  onResumeUnregister =
                    SteamClient.System.RegisterForOnResumeFromSuspend(() => {
                      AsyncUtils.runMutexForProfile((release) => {
                        Logger.info(
                          "Waiting 10 seconds for restoring CPU profile",
                        );
                        sleep(10000).then(() => {
                          BackendUtils.setPerformanceProfile(
                            Profiles.getProfileForId(
                  ...
                  BackendUtils.setBatteryLimit(
                    SystemSettings.getLimitBattery(),
                  );
                  if (WhiteBoardUtils.getIsAllyX()) {
                    sleep(100).then(() => {
                      Profiles.summary();
