...
export class BackendUtils {
  ...
  public static async setPerformanceProfile(profile: Profile): Promise<void> {
    if (WhiteBoardUtils.getIsAllyX()) {
      ...
      Backend.backend_call<[prof: string], number>(
  ...
  public static async getGpuFrequencyRange(): Promise<[number, number]> {
    return Backend.backend_call<[], [number, number]>(
      "get_gpu_frequency_range",
    );
  }

  public static async getIconForApp(appId: string): Promise<string | null> {
    return Backend.backend_call<[appId: string], string>(
      "get_icon_for_app",
      appId,
    );
  }
